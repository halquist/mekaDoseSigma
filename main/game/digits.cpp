...
void drawGlyph(uint16_t* framebuffer, int screenWidth, int screenHeight,
               int digit, int leftX, int topY, int scale, uint16_t packedColor) {
    if (digit < 0 || digit > 9) {
        return;
    }

    for (int row = 0; row < 5; ++row) {
        const uint8_t bits = kDigitRows[digit][row];
        for (int col = 0; col < 4; ++col) {
            ...
            for (int sy = 0; sy < scale; ++sy) {
                ...
                for (int sx = 0; sx < scale; ++sx) {
                    const int px = leftX + col * scale + sx;
                    if (px < 0 || px >= screenWidth) {
                        continue;
                    }
...
void Digits::drawDigit(uint16_t* framebuffer, int screenWidth, int screenHeight,
    ...
    const int glyphW = 4 * scale;
    const int glyphH = 5 * scale;
    drawGlyph(framebuffer, screenWidth, screenHeight, digit,
              cx - glyphW / 2, cy - glyphH / 2, scale, fbPack(color));
}
...
void Digits::drawNumber(uint16_t* framebuffer, int screenWidth, int screenHeight,
    ...
    for (int i = 0; i < count; ++i) {
        x -= glyphW;
        drawGlyph(framebuffer, screenWidth, screenHeight, digits[i],
                  x, cy - (5 * scale) / 2, scale, fbPack(color));
        x -= spacing;
    }
}
